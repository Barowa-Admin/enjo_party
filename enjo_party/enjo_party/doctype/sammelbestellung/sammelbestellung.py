        ...
        if created_orders:
            ...
            for order_name in created_orders:
                try:
                    ...
                    frappe.log_error(f"Prüfe Sales Order {so_doc.name} für Partner-Auftrag", "DEBUG: check_so")

                    # Finde das Versandziel für diesen Kunden
                    ...
                    for idx, kunde_row in enumerate(sammelbestellung_doc.kunden or []):
                        if kunde_row.kunde == so_doc.customer:
                            ...
                            if hasattr(sammelbestellung_doc, versand_field):
                                shipping_target = getattr(sammelbestellung_doc, versand_field)
                                frappe.log_error(f"Versandziel gefunden für {so_doc.customer}: {shipping_target}", "DEBUG: found_shipping_target")
                            break

                    # Wenn ein anderes Versandziel gefunden wurde, zur Gruppe hinzufügen
                    if shipping_target and shipping_target != so_doc.customer:
                        if shipping_target not in shipping_groups:
                            shipping_groups[shipping_target] = []
                        shipping_groups[shipping_target].append(so_doc)
                        frappe.log_error(f"Auftrag {so_doc.name} zu Gruppe {shipping_target} hinzugefügt", "DEBUG: add_to_group")

                except Exception as e:
                    frappe.log_error(f"Fehler beim Gruppieren des Auftrags {order_name}: {str(e)}", "ERROR: group_order_error")
                    continue

            # Erstelle einen Partner-Auftrag pro Versandziel
            for shipping_target, orders in shipping_groups.items():
                try:
                    frappe.log_error(f"Erstelle Partner-Auftrag für {shipping_target} mit {len(orders)} Kunden", "DEBUG: create_partner")

                    # Hole den Dummy-Artikel "Partnerversand"
                    dummy_item_code = "Partnerversand"
                    dummy_item = frappe.get_doc("Item", dummy_item_code)

                    # Finde Partner-Adresse
                    partner_address = find_existing_address(shipping_target, "Billing")
                    if not partner_address:
                        partner_address = find_existing_address(shipping_target, "Shipping")

                    # Prüfe ob tatsächlich Produkte an den Partner gehen
                    has_products = False
                    for order in orders:
                        if any(item.item_code != dummy_item_code for item in order.items):
                            has_products = True
                            break

                    if not has_products:
                        frappe.log_error(f"Keine Produkte für Partner {shipping_target} gefunden - überspringe", "INFO: skip_partner")
                        continue
                    ...
                    partner_order_data = {
                        ...
                        "remarks": f"Versandauftrag erstellt aus Sammelbestellung | Partner: {shipping_target}",
                        "po_no": f"{sammelbestellung}-SHIP-{shipping_target}",
                    ...
                    frappe.log_error(f"Partner-Auftrag erstellt: {partner_order.name}", "SUCCESS: partner_created")

                    # Erstelle und buche die Ausgangsrechnung
                    from erpnext.selling.doctype.sales_order.sales_order import make_sales_invoice
                    invoice = make_sales_invoice(partner_order.name)
                    invoice.flags.ignore_permissions = True  # Wichtig für das Buchen
                    invoice.update_stock = 0  # Kein Lagerbestand aktualisieren
                    invoice.insert()
                    invoice.submit()
                    frappe.log_error(f"Ausgangsrechnung erstellt und gebucht: {invoice.name}", "SUCCESS: invoice_created")

                except Exception as e:
                    frappe.log_error(f"Fehler beim Erstellen des Partner-Auftrags für {shipping_target}: {str(e)}", "ERROR: partner_order_failed")
                    continue
